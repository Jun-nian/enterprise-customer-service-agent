...
from utils.llms import get_llm, get_embedding_dims
# 导入工具配置模块
...
class MessagesState(TypedDict):
    ...
    # P0-A 规划范式: 规划模式（simple=快路径 / complex=多步执行）
    plan_mode: Annotated[Optional[str], "Planning mode: 'simple' or 'complex'"]
    # P0-A 规划范式: 结构化计划步骤列表 [{tool, query, goal, ...}]
    plan: Annotated[Optional[list], "Structured plan steps produced by planner"]
    # P0-A 规划范式: 当前执行到的步骤索引
    plan_index: Annotated[Optional[int], "Current plan step index"]
    # P0-A 规划范式: 各步骤执行观察结果 [{tool, output, ok, empty}]
    plan_observations: Annotated[Optional[list], "Per-step observations collected by executor/critic"]
    # P0-A 规划范式: 动态重规划次数
    plan_attempts: Annotated[Optional[int], "Number of replan attempts"]
    # P0-A 规划范式: critic 的下一次路由决策（executor/generate/planner）
    next: Annotated[Optional[str], "Next routing decision from critic"]
    # P0-C 安全护栏: 输入侧是否被拦截（true=攻击命中已拒答）
    guard_blocked: Annotated[Optional[bool], "Whether input was blocked by guardrails"]
    # P0-C 安全护栏: 输入侧拦截的攻击类型
    guard_attack: Annotated[Optional[str], "Attack type detected by input guard"]
...
class ParallelToolNode(ToolNode):
    ...
    def _run_single_tool(self, tool_call: dict, tool_map: dict) -> ToolMessage:
        """执行单个工具调用（P1-B: 经可靠性包装器执行 超时/重试/降级/熔断）"""
        ...
        try:
            ...
            # P1-B 工具可靠性：开启时用包装器执行（超时/重试/降级/熔断），否则直接调用
            if Config.ENABLE_RELIABILITY:
                from reliability.tool_wrapper import wrap_tool, ToolReliabilityConfig
                _cfg = ToolReliabilityConfig(
                    timeout_seconds=Config.TOOL_TIMEOUT_SECONDS,
                    max_retries=Config.TOOL_MAX_RETRIES,
                )
                wrapped = wrap_tool(tool, _cfg)
                result = wrapped.invoke(tool_call["args"])
            else:
        ...
        if template_file in create_chain.prompt_cache:
            ...
            with create_chain.lock:
                ...
                if template_file not in create_chain.prompt_cache:
                    ...
                    # 从文件加载提示模板，显式指定 UTF-8 编码
                    with open(template_file, "r", encoding="utf-8") as f:
                        create_chain.prompt_cache[template_file] = PromptTemplate.from_template(f.read())
                # 从缓存中获取提示模板
                prompt_template = create_chain.prompt_cache[template_file]
...
def generate(state: MessagesState, llm_chat) -> dict:
    ...
    try:
        ...
        # P0-A 规划范式: 复杂模式优先使用多步计划的观察结果作为上下文
        context = None
        if Config.ENABLE_PLANNING and state.get("plan_mode") == "complex":
            observations = state.get("plan_observations") or []
            if observations:
                parts = []
                for obs in observations:
                    parts.append(f"[步骤{obs.get('step_index', '?')} - {obs.get('tool', '?')}]\n{obs.get('output', '')}")
                context = "\n\n".join(parts)
                logger.info(f"[generate] 使用 {len(observations)} 条计划观察作为上下文")

        # 兜底：从最后一条消息获取上下文(因为调用工具输出的内容写入到state的最新消息中)
        if context is None:
            context = state["messages"][-1].content
...
def route_after_tools(state: MessagesState, tool_config: ToolConfig) -> Literal["generate", "grade_documents", "critic"]:
    """
    ...
    Returns:
        Literal["generate", "grade_documents", "critic"]: 下一步的目标节点。
    """
    # P0-A 规划范式: 复杂模式（executor 执行的多步计划）→ 交给 critic 评估，不走检索评分快路径
    if Config.ENABLE_PLANNING and state.get("plan_mode") == "complex":
        logger.info("Planning mode complex, routing tool result to critic")
        return "critic"
...
# P0-C 安全护栏: 输入侧护栏路由（拦截→END，通过→planner(规划开启)/agent(快路径)）
def _route_after_guard_input(state: MessagesState):
    """根据 guard_blocked 决定下一步（返回 END 或节点名）"""
    # 优先读状态标记；若条件边读不到（版本差异），回退判断最后一条消息是否为拦截响应
    blocked = state.get("guard_blocked")
    if not blocked:
        msgs = state.get("messages") or []
        if msgs:
            last = msgs[-1]
            content = str(getattr(last, "content", ""))
            if "已被安全策略拦截" in content:
                blocked = True
    if blocked:
        logger.info("[guard_input] 输入被拦截，路由到 END")
        return END
...
def create_graph(llm_chat, llm_embedding, tool_config: ToolConfig) -> StateGraph:
    ...
    # 跨线程持久化存储 (内存)，动态适配嵌入维度
    dims = get_embedding_dims(llm_embedding)
    store = InMemoryStore(
        index={"dims": dims, "embed": llm_embedding}
    )

    # 创建状态图实例，使用MessagesState作为状态类型
    workflow = StateGraph(MessagesState)

    # P0-A 规划范式: 开启时注入 planner/executor/critic 节点（复杂任务多步执行 + 动态重规划）
    if Config.ENABLE_PLANNING:
        from planning.planner import planner_node, route_after_planner
        from planning.executor import executor_node
        from planning.critic import critic_node, route_after_critic

        workflow.add_node("planner", lambda state: planner_node(state, llm_chat=llm_chat))
        workflow.add_node("executor", lambda state: executor_node(state, tool_config=tool_config))
        workflow.add_node("critic", lambda state: critic_node(state, llm_chat=llm_chat, tool_config=tool_config))

        # START → planner（规划器先判定复杂度）；护栏开启时 START 由 guard_input 接管，经条件边路由到 planner
        if not Config.ENABLE_GUARDRAILS:
            workflow.add_edge(START, end_key="planner")
        # planner → agent(快路径) 或 executor(多步执行)
        workflow.add_conditional_edges(source="planner", path=route_after_planner,
                                       path_map={"agent": "agent", "executor": "executor"})
        # executor 每步产出 tool_call → call_tools 执行
        workflow.add_edge(start_key="executor", end_key="call_tools")
        # critic 决策：executor(继续) / generate(完成) / planner(重规划)
        workflow.add_conditional_edges(source="critic", path=route_after_critic,
                                       path_map={"executor": "executor", "generate": "generate", "planner": "planner"})
    ...
    # P0-C 安全护栏: 开关开启时注入 guard_input（输入检测）/ guard_output（输出脱敏）节点
    if Config.ENABLE_GUARDRAILS:
        from guardrails.input_guard import guard_input, BLOCK_RESPONSE
        from guardrails.output_guard import guard_output

        # 输入侧护栏：START 之后第一个节点，命中攻击范式直接拒答
        def _guard_input_node(state):
            from demoRagAgent import get_latest_question
            question = get_latest_question(state) or ""
            result = guard_input(question)
            if not result.passed:
                logger.warning(f"[guard_input] 拦截输入: {result.action} ({result.attack_type})")
                from langchain_core.messages import AIMessage
                return {
                    "messages": [AIMessage(content=result.response or BLOCK_RESPONSE)],
                    "guard_blocked": True,
                    "guard_attack": result.attack_type,
                }
            return {"messages": [], "guard_blocked": False, "guard_attack": None}

        # 输出侧护栏：generate 产出后对回答做 PII 脱敏
        def _guard_output_node(state):
            from langchain_core.messages import AIMessage
            msgs = list(state.get("messages") or [])
            if msgs:
                last = msgs[-1]
                if hasattr(last, "content") and last.content:
                    masked = guard_output(str(last.content))
                    if masked != last.content:
                        new_msg = AIMessage(content=masked, id=getattr(last, "id", None))
                        msgs[-1] = new_msg
            return {"messages": msgs}

        workflow.add_node("guard_input", _guard_input_node)
        workflow.add_node("guard_output", _guard_output_node)
        # guard_input 有条件边：拦截→END，通过→planner(规划开启)/agent(快路径)
        _guard_input_path_map = {END: END, "agent": "agent"}
        if Config.ENABLE_PLANNING:
            _guard_input_path_map["planner"] = "planner"
        workflow.add_conditional_edges(
            source="guard_input",
            path=lambda state: _route_after_guard_input(state),
            path_map=_guard_input_path_map,
        )
        # generate → guard_output → END
        workflow.add_edge(start_key="generate", end_key="guard_output")
        workflow.add_edge(start_key="guard_output", end_key=END)
    ...
    # 添加从起始到代理的边（未开启规划时保持原 START→agent 快路径）
    if not Config.ENABLE_PLANNING and not Config.ENABLE_GUARDRAILS:
        workflow.add_edge(START, end_key="agent")
    # 护栏开启时：START → guard_input（后续条件路由到 planner/agent）
    if Config.ENABLE_GUARDRAILS:
        workflow.add_edge(START, end_key="guard_input")
    ...
    # 规划开启时 call_tools 可路由到 critic（复杂模式），否则仅 generate/grade_documents
    if Config.ENABLE_PLANNING:
        workflow.add_conditional_edges(source="call_tools", path=lambda state: route_after_tools(state, tool_config),
                                       path_map={"generate": "generate", "grade_documents": "grade_documents", "critic": "critic"})
    else:
        workflow.add_conditional_edges(source="call_tools", path=lambda state: route_after_tools(state, tool_config),
...
def main():
    ...
    try:
        ...
        # 打印机器人就绪提示
        print("企业智能客服已就绪！输入 'quit'、'exit' 或 'q' 结束对话。")
        print("您可以查询：员工信息、公司FAQ、订单状态、IT工单、企业知识库等。")
        # 定义运行时配置，包含线程ID和用户ID

...
# 企业智能客服 - FastAPI 服务入口
# 基于 LangGraph + ChromaDB + FastAPI + Gradio
            ...
            for message_chunk, metadata in stream_data:
                try:
                    ...
                    if node_name == "generate":
                        ...
                        # agent 节点：只有不调用工具时（直接回答）才展示，调用工具时是推理过程需隐藏
                        has_tool_calls = (
                            hasattr(message_chunk, 'tool_calls') and message_chunk.tool_calls
                        )
                        if not has_tool_calls:
                            chunk = getattr(message_chunk, 'content', '')
                            if chunk:
                                logger.info(f"Streaming chunk from agent (direct): {chunk}")
                                yield f"data: {json.dumps({'id': chunk_id, 'object': 'chat.completion.chunk', 'created': int(time.time()), 'choices': [{'index': 0, 'delta': {'content': chunk}, 'finish_reason': None}]})}\n\n"

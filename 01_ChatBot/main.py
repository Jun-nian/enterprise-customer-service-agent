...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        ...
        with open(PROMPT_TEMPLATE_TXT_SYS, "r", encoding="utf-8") as f:
            sys_content = f.read()
        with open(PROMPT_TEMPLATE_TXT_USER, "r", encoding="utf-8") as f:
            user_content = f.read()
        prompt = [
            {"role": "system", "content": sys_content},
            {"role": "user", "content": user_content.format(query=query_prompt)}
        ]
